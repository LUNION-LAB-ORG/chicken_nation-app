...
const HomeBanner = () => {
  ...
  const handleCardPress = (cardAnimation, cardIndex) => {
    if (activeCard === cardIndex) {
      // Si la carte est déjà révélée, naviguer vers l'écran correspondant
      if (isCardRevealed === cardIndex) {
        switch (cardIndex) {
          case 1:
            router.push("/(tabs-guest)/delivery");
            break;
          case 2:
            router.push("/(tabs-guest)/takeaway");
            break;
          case 3:
            router.push("/(tabs-guest)/reservation");
            break;
        }
        return;
      }
      // Marquer la carte comme révélée
      setIsCardRevealed(cardIndex);
    } else {
      // Réinitialiser l'état révélé et activer la nouvelle carte
      setIsCardRevealed(null);
      setActiveCard(cardIndex);
    }
  ...
  // Obtenir les transformations d'animation pour chaque carte
      ...
      transform: [
        {
          translateY: animation.interpolate({
            inputRange: [0, 1],
            outputRange: [0, -10], // Remonte légèrement le contenu quand actif
          }),
        },
      ],
    };
  ...
  // Get blur radius based on active state - only blurred when inactive
            ...
            >
              ...
              >
                ...
                <View style={styles.cardContentWrapper}>
                  {/* Image d'arrière-plan avec blur pour les cartes inactives */}
                  <Image
                    source={require("../../assets/images/homebanner.png")}
                    style={[
...
const styles = StyleSheet.create({
  ...
  cardWrapper: {
    width: CARD_WIDTH,
    height: 230,
    borderRadius: 100, // Bordure très arrondie
    overflow: "hidden",
  },
  cardContentWrapper: {
    width: "100%",
    height: "100%",
    overflow: "hidden",
    borderRadius: 100, // Bordure très arrondie (matching cardWrapper)
    position: "relative",
  },
  blurBackground: {
    position: "absolute",
